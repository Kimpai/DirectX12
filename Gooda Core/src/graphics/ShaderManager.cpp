...
namespace GoodaCore
{
	...
	{
		ID3D12GraphicsCommandList* commandList = Direct3D12::Instance()->GetCommandList();

		commandList->SetPipelineState(m_pipelines[0]->GetPipelineState());
	...
	void ShaderManager::CreatePipelineState(std::vector<Shader> shaders, ShaderPipelineType type)
	{
		...
		m_pipelines.push_back(new PipelineState(Direct3D12::Instance()->GetDevice(), shaders, type, &depthStencilDesc, m_rootSignature->GetRootSignature()));
	}
	...
	void ShaderManager::CreateRootSignature()
	{
		...
		m_rootSignature = new RootSignature(Direct3D12::Instance()->GetDevice(), (UINT)m_rootParameters.size(), 0, m_rootParameters[0], NULL, D3D12_ROOT_SIGNATURE_FLAG_ALLOW_INPUT_ASSEMBLER_INPUT_LAYOUT);
	...
	void ShaderManager::CreateDepthStencil(D3D12_DEPTH_STENCIL_DESC& depthStencilDesc)
	{
		...
		assert(!Direct3D12::Instance()->GetDevice()->CreateDescriptorHeap(&depthStencilViewHeapDesc, __uuidof(ID3D12DescriptorHeap), (void**)m_depthStencilDescHeap.GetAddressOf()));
		...
		assert(!Direct3D12::Instance()->GetDevice()->CreateCommittedResource(&CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_DEFAULT), D3D12_HEAP_FLAG_NONE,
		...
		Direct3D12::Instance()->GetDevice()->CreateDepthStencilView(m_depthStencilBuffer.Get(), &depthStencilViewDesc, m_depthStencilDescHeap->GetCPUDescriptorHandleForHeapStart());
	}
	...
	{
		m_mainDescriptorHeap = new DescriptorHeap(Direct3D12::Instance()->GetDevice(), (UINT)m_constantBuffers.size(), true, D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
